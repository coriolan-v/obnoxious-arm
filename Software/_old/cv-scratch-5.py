...
groupSyncReadElbow = GroupSyncRead(packetHandler, SMS_STS_PRESENT_POSITION_L, 11) # Initialize sync read for elbow
...
min_x = float('inf')
max_x = float('-inf')
min_y = float('inf')  # Initialize min_y
max_y = float('-inf')  # Initialize max_  

scs_comm_result, scs_error = packetHandler.WritePosEx(SCS_SHOULDER_ID, SCS_SHOULDER_HOME_POSITION_VALUE, SCS_MOVING_SPEED, SCS_MOVING_ACC)
...
print(f"Motor moved to home position: {SCS_SHOULDER_HOME_POSITION_VALUE}")
time.sleep(2) # Give it time to get there

scs_comm_result, scs_error = packetHandler.WritePosEx(SCS_ELBOW_ID, SCS_ELBOW_HOME_POSITION_VALUE, SCS_MOVING_SPEED, SCS_MOVING_ACC)
if scs_comm_result != COMM_SUCCESS:
    print(f"Motor home command failed: {packetHandler.getTxRxResult(scs_comm_result)}")
if scs_error != 0:
    print(f"Motor home error: {packetHandler.getRxPacketError(scs_error)}")
print(f"Motor moved to home position: {SCS_ELBOW_HOME_POSITION_VALUE}")
...
while True:
    ...
    for tracker in trackers:
        ...
        # 1. Read Current _SHOULDER_ Motor Position
        ...
        if scs_data_result == True:
            current_shoulder_position = groupSyncRead.getData(SCS_SHOULDER_ID, SMS_STS_PRESENT_POSITION_L, 2)
            print(f"Current Shoulder Position: {current_shoulder_position}")
            ...
            target_shoulder_position = current_shoulder_position + motor_adjustment  # Use current position
            ...
            if target_shoulder_position != current_shoulder_position:
                scs_comm_result, scs_error = packetHandler.WritePosEx(SCS_SHOULDER_ID, target_shoulder_position, SCS_MOVING_SPEED, SCS_MOVING_ACC)
                if scs_comm_result != COMM_SUCCESS:
                ...
                print(f"Target Shoulder Position: {target_shoulder_position}")
                
        else:
        #print(f"[ID:{SCS_SHOULDER_ID}] groupSyncRead getdata failed")
        # Handle the case where reading fails (e.g., set current_shoulder_position to a default or skip motor control)
        ...
        # ... (Inside the if trackers: block, after calculating remapped_y)

        center_y = resized_frame.shape[0] // 2  # Center of the y-axis (height)
        tolerance_y = 50  # Reduced tolerance for y-axis

        groupSyncReadElbow.clearParam()
        scs_addparam_result = groupSyncReadElbow.addParam(SCS_ELBOW_ID)
        if scs_addparam_result != True:
            print(f"[ID:{SCS_ELBOW_ID}] groupSyncRead addparam failed")

        scs_comm_result = groupSyncReadElbow.txRxPacket()
        if scs_comm_result != COMM_SUCCESS:
            print(f"SyncRead Error: {packetHandler.getTxRxResult(scs_comm_result)}")

        scs_data_result, scs_error = groupSyncReadElbow.isAvailable(SCS_ELBOW_ID, SMS_STS_PRESENT_POSITION_L, 2)

        if scs_data_result == True:
            current_elbow_position = groupSyncReadElbow.getData(SCS_ELBOW_ID, SMS_STS_PRESENT_POSITION_L, 2)
            print(f"Current Elbow Position: {current_elbow_position}")
            ...
            # Move the motor if the error exceeds the tolerance
            if abs(error_y) > tolerance_y:
                scs_comm_result, scs_error = packetHandler.WritePosEx(SCS_ELBOW_ID, target_elbow_position, SCS_MOVING_SPEED, SCS_MOVING_ACC)  # <--- THIS WAS MISSING!
                if scs_comm_result != COMM_SUCCESS:
                    print(f"Elbow motor command failed: {packetHandler.getTxRxResult(scs_comm_result)}")
                if scs_error != 0:
                    print(f"Elbow motor error: {packetHandler.getRxPacketError(scs_error)}")
                print(f"Target Elbow Position: {target_elbow_position}")
                print(f"Current Elbow Position: {current_elbow_position}")
            else:
                print("Error within tolerance - motor not moved")
